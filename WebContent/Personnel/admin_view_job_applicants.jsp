...
<html>
	...
	<div class="panel-group" style="padding-top: 5px;">
		<div class="panel panel-success">
			<div class="panel-heading">
				<b>Competition # <%=job.getCompetitionNumber()%></b> (Total Applicants: <%=applicants.length%>)
			</div>
			<div class="panel-body">
				...
				<div class="no-print" align="center">
					...
					<!--  PRINT JOB -->
					<a class="btn btn-xs btn-primary" href='#' title='Print this page (pre-formatted)' class="btn btn-xs btn-primary"
				...
				<div class="table-responsive" style="font-size:11px;">
    								...
    								<table class="table table-condensed table-striped" style="font-size: 11px; background-color: #FFFFFF;">
    									<tbody>
    										<tr>
	    										...
	    										<td width="20%">${sfilterparams.mathCourses }</td>    										
    										</tr>
    										<tr>
    											<td width="30%">Permanent Exp (Months):</td>
    											<td width="20%">${sfilterparams.permanentExp }</td>
    											<td width="30%"># English Courses:</td>
    											<td width="20%">${sfilterparams.englishCourses }</td>   										
    										</tr>
    										<tr>
    											<td width="30%">Replacement Exp (Months):</td>
    											<td width="20%">${sfilterparams.replacementExp }</td>
    											<td width="30%"># Music Courses:</td>
    											<td width="20%">${sfilterparams.musicCourses }</td>    										
    										</tr>
    										<tr>
    											<td width="30%">Repl + Perm (Months):</td>
    											<td width="20%">${sfilterparams.totalExp }</td>
    											<td width="30%"># Technology Courses:</td>
    											<td width="20%">${sfilterparams.technologyCourses }</td>    										
    										</tr>
    										<tr>
    											<td width="30%"># Sub Days:</td>
	    										<td width="20%">${sfilterparams.subDays }</td>
	    										<td width="30%"># Science Courses:</td>
	    										<td width="20%">${sfilterparams.scienceCourses }</td>
    										</tr>
    										<tr>
					...
					<% if (applicants.length > 0) { %>
						<p>
							Below is a table of current applicants for this position sorted by seniority. 
							<% if (job.getIsSupport().equals("N")) { %>
								The type and certificates/courses are displayed for quick reference for <b>non support staff/management positions</b>.
								(UT = #University Transcripts, TC = #Teaching Certificates, COC = #Code of Conducts, VSC = #Vulnerable Sector Checks, 
								FPD = #French Proficiency (DELF), CEC = #Level 2 Early Childhood Education Certificates , and #CRS = # of Courses)
							<% } %> 
						</p>
						...
						<table id="jobsapp" class="table table-condensed table-striped" style="font-size: 11px; background-color: #FFFFFF;">
							...
							<tbody>
								...
								<tr class='applicant-list<%=(isLabWestSchool ? " lab-west-school" : "")%>' uid='<%=applicants[i].getSIN()%>'>	
									...
									<% if ((applicants[i].getProfileType().equals("T") && job.getIsSupport().equals("Y")) 
											|| (applicants[i].getProfileType().equals("S") && job.getIsSupport().equals("N"))) { %>	
	                ...
	                <!-- #Courses -->           
									...
									<% } %>
								<% } else if (applicants[i].getProfileType().equals("S")) { %>
								...
								<td style="text-align:right;">		
									<a  class='btn btn-xs btn-primary' href="viewApplicantProfile.html?sin=<%=applicants[i].getSIN()%>">Profile</a>
										<% if (usr.checkRole("ADMINISTRATOR") || usr.checkRole("MANAGER OF HR - PERSONNEL")) { %> 
											<a href="#" data-toggle="confirmation" class="btn btn-danger btn-xs" 
													comp-num='<%=job.getCompetitionNumber()%>' uid='<%=applicants[i].getSIN()%>'
													data-title="Are you sure you wish to withdraw <%=applicants[i].getFullNameReverse()%> from this competition?">Withdraw</a> 
										<% } %>
								</td>
							</tr>
								<% } %>
	...
	<div class="no-print" align="center">
		<a onclick="loadingData()" class="btn btn-xs btn-info" href='view_job_post.jsp?comp_num=<%=job.getCompetitionNumber()%>'>View Job</a> 
		<a class="btn btn-xs btn-primary" href='#' title='Print this page (pre-formatted)' class="btn btn-xs btn-primary"
			onclick="jQuery('#printJob').print({prepend : '<div align=center style=margin-bottom:15px;><img width=400 src=includes/img/nlesd-colorlogo.png><br/><br/><b>Human Resources Profile System</b></div><br/><br/>'});">
				...
				<div class="modal-body">
					<form name="email" action="" method="post" onsubmit="return confirm_send();">
						...
						<div class="form-group">
							<label for="from">From:</label> <input class="form-control" type="text" name="from" value="<%=usr.getPersonnel().getEmailAddress()%>" readonly="readonly">
						</div>

						<div class="form-group">
							<label for="to">To:</label> <input class="form-control" type="text" name="to">
						</div>

						<div class="form-group">
							<label for="cc">Cc:</label> <input class="form-control" type="text" name="cc">
						</div>
						...
						<div class="form-group">
							<label for="subject">Subject:</label> <input type="text" name="subject" class="form-control" value='NLESD Employment Opportunity System'>
						</div>
