...
<%if(applicants.length > 0) { %>
  ...
  <%for(int j=0; j < applicants.length; j++){
      ...
      regionPrefs = ApplicantRegionalPreferenceManager.getApplicantRegionalPreferencesMap(applicants[j]).values().toArray(new RegionBean[0]);
      per = ApplicantNLESDPermExpManager.getApplicantNLESDPermanentExperienceBeans(applicants[j].getSIN());
  %>
...
<!-- APPLICANT PROFILE-->		
		...
		<%=applicants[j].getFirstname() + " " + ((applicants[j].getMiddlename() != null)?applicants[j].getMiddlename() + " ":"") + applicants[j].getSurname() 
		          + ((applicants[j].getMaidenname()!= null)?" ("+applicants[j].getMaidenname()+")":"")%>
		...
		<td class="tableResult" colspan=3>
					<%=applicants[j].getAddress1()%>		            
		             <%=(applicants[j].getAddress2()!=null)?" &middot; "+applicants[j].getAddress2():""%>		            
		            , <%=applicants[j].getProvince() + " &middot: " + applicants[j].getCountry() + " &middot; " + applicants[j].getPostalcode()%>
		</td>
		</tr>
		<tr>
		<td class="tableTitle" colspan=1>TELEPHONE: </td>
		<td class="tableResult" colspan=3>
		(Res) <%=(applicants[j].getHomephone()!=null)?applicants[j].getHomephone():"N/A"%> &middot; 
		(Work) <%=(applicants[j].getWorkphone()!=null)?applicants[j].getWorkphone():"N/A"%> &middot; 
		(Cell) <%=(applicants[j].getCellphone()!=null)?applicants[j].getCellphone():"N/A"%>
		</td>
		</tr>
		<tr>
...
<!-- REGIONAL REFERENCES --------------------------------------------------------------->
	              ...
	              <table class="table table-striped table-condensed" id="regPrefs" style="font-size:10px;">
      							    <thead>
      							    <tr>                                       
                                       <th colspan=2 style="font-weight:bold;border-top:1px solid black;">REGIONAL PREFERENCES</th>                                
                                      </tr>
                                      </thead>
                                      <tbody>  
					...
					<%if((regionPrefs != null) && (regionPrefs.length > 0)) { %>
                       ...
                       <% } %>
                    	...
                    	<%} else { %>
                    		 <tr>
							    <td colspan=2><span style="color:Grey;">No References currently on file.</span></td>
							</tr>                  		 
                    	<%} %>
