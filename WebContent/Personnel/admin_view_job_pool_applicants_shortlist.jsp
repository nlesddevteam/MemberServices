										...
										<div class="panel panel-default">
										  <div class="panel-heading">
										    <h4 class="panel-title">
										      <a data-toggle="collapse" data-parent="#accordion" href="#collapse<%=regionNum%>">
										     <%= region %> <span  id='regiontotal_<%=region.replaceAll(" ", "").replaceAll("/","-")%>'></span></a>
										    </h4>
										  </div>
										  <div id="collapse<%=regionNum%>" class="panel-collapse collapse">
							<div class="panel-body">
										    Applicants listed below are initially sorted based on Days of seniority. Click on any header to sort by that category.<br/><br/>
									    ...
									    <thead>									    
									      <tr>
									        ...
									        <th width='20%'>EMAIL/TELEPHONE</th>
									        <th width="10%">STATUS</th>				        
									        <th width='40%'>POSITION/OPTIONS</th>									       					        
									      </tr>
									    </thead>
									    <tbody>
										    
										    
									<%for(int days : regions.get(region).keySet()){
                                  			applicants = (ApplicantProfileBean[]) regions.get(region).get(days).toArray(new ApplicantProfileBean[0]); 
                                  			totalregion += applicants.length;
                                  ...
                                  <%if(applicants.length > 0){         	  
                                    ...
                                    %>	
	                                    	...
	                                    	<td style="vertical-align:middle;"><%=applicants[i].getFirstname()%></td>
	                                    	<td style="vertical-align:middle;"><%=applicants[i].getSurname()%></td>	                                    	
	                                        ...
	                                        <esd:SecurityAccessRequired permissions="PERSONNEL-ADMIN-VIEW">
	                                        	<% 
	                                        		if(!interviewSummaryMap.containsKey(applicants[i].getUID())) {
	                                        			out.println("<a class='btn btn-xs btn-primary' href='addInterviewSummary.html?applicant_id=" + applicants[i].getUID() 
                                 ...
                                 <%} 
                                  	...
                                  	<script type="text/javascript">$('#regiontotal_<%=region.replaceAll(" ", "").replaceAll("/","-")%>').text('(Applicants: <%=totalregion%>)');</script>	
                                  <%}%>
                                  </div></div></div></div></div>
