...
function checknlesdexp() {
	...
	if(selected_value == "Y"){
		if (permschool == "0") {
			$("#spanmsg").html("Work Location is a required field");
			$("#divmsg").show();
			return false;
		}
		if (unioncode == "-1") {
		...
		if (sdate == "") {
			$("#spanmsg").html("Start Date is a required field");
			$("#divmsg").show();
			return false;
		}
		$("#hidadd").val("ADDNEW");
...
function checkneweducation() {
	...
	$("#divmsg").hide();
	//return true;
	//add value to tell handler what to update
	
	$("#ApplicantRegistrationStep1").attr('action', 'applicantRegistrationSS.html?step=5a');
