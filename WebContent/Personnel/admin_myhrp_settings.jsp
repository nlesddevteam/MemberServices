...
<body>
	...
	<c:choose>
		<c:when test="${ppBlockSchools eq true}">
			<script>$("#ppStatus").css("display","block").addClass("alert-warning").removeClass("alert-danger").html("<b>WARNING:</b> Position Planner is DISABLED for School Administrators.");</script>
		</c:when>
		<c:when test="${ppBlockSchools eq false}">
			<script>$("#ppStatus").css("display","block").addClass("alert-success").removeClass("alert-danger").html("Position Planner for School Administrators is currently ACTIVE.");</script>
		</c:when>
		<c:otherwise>
