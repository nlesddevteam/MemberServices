...
.tableResultL {font-weight:normal;width:35%;}
    ...
    <div class="panel-group" style="padding-top:5px;">                               
	               	...
	               	<div class="panel-heading"><b>View Interview Summary</b> for Competition #<%= summary.getCompetition().getCompetitionNumber() %></div>
                     ...
                     <table class="table table-striped table-condensed" style="font-size:11px;">	
	                                    ...
	                                    <tr>
	                              		...
	                              		<%= guide.getQuestionCount() %> questions - Each ranked from <%= guide.getRatingScaleBottom() %> to <%= guide.getRatingScaleTop() %>, with <%= guide.getRatingScaleTop() %> being outstanding.
	                                    ...
	                                    for(int i=1; i <= guide.getQuestionCount(); i++) { %>
                               			<th width="10%" style="text-align:center;">Q/C<%= i %>(<%= (guide.getQuestions().get(i-1).getWeight()) %>)</th>
                               			<%
                               			perCalc = perCalc + guide.getQuestions().get(i-1).getWeight();
                               			numQ++; 
                               			} %>
                               			<th width="10%;" style="text-align:center;">Weighted<br />Total Score</th>
                               			<th width="10%;" style="text-align:center;">Weighted<br />Avg. Score</th>
	                                    </tr>
	                                    </thead>			
	                                    <tbody>	
	                                    <%
	                                    ...
	                                    if(summary.getInterviewSummaryScoreBeans() != null && summary.getInterviewSummaryScoreBeans().size() > 0){
                                    						...
                                    						for(InterviewSummaryScoreBean iss : summary.getInterviewSummaryScoreBeans()) {
																...
																<td style="text-align:center;"><%tdnum++; %><%=(((score > 0)&&(question.getWeight() > 0)) ? Double.toString(score) : "0")%></td>
	                                    				...
	                                    				<tr>
	                                    				<td style="font-weight:bold;" colspan="<%=tdnum+2%>">Average Score</td>
	                                    				<td style="font-weight:bold;text-align:center;"><%= df.format(summary.getOverallScore(guide))%></td>
	                                    				</tr>
	                                    				<tr>
	                                    				<td style="font-weight:bold;" colspan="<%=tdnum+2%>">Percentage: </td>
	                                    				<td style="font-weight:bold;text-align:center;">
	                                    				<%double roundOff = Math.round((summary.getOverallScore(guide)/perCalc) * 100.0); 
	                                    				if (roundOff <= 50) { %>
	                                    				<span style="color:Red;"><%=roundOff%>%</span>
	                                    				<% } else if (roundOff >50 && roundOff < 80){ %>
	                                    				<span style="color:Orange;"><%=roundOff%>%</span>
	                                    				<% } else if (roundOff >=80){ %>
	                                    				<span style="color:Green;"><%=roundOff%>%</span>
	                                    				<%} else {%>
	                                    				<span style="color:Black;"><%=roundOff%>%</span>
	                                    				<%} %>
	                                    				<script>$("#scorePc").css("display","block").html(<%=roundOff%>+"%");</script>
	                                    				
	                                    				</td>
	                                    				</tr>
	                                    			<%}else{%>	                                    				
		                                    			<tr><td colspan="<%=tdnum+3%>">No interviewer/scoring information found.</td></tr>
		                                    		<%}%>
	                                    		</tbody>	
	                                    		</table>
