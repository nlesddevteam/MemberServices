...
$(document).ready(function() {
	...
	<div id="printJob">	
		 <div class="BCSHeaderText">View Route/Run Drivers/Vehicles</div>		 
			  <br />
    		...
    		<div class="alert alert-success" id="details_success_message" style="display:none;margin-top:10px;margin-bottom:10px;padding:5px;"></div>
			 ...
			 To export this data to Excel, CSV or to print/copy, you can select either option below to download a copy to open or print.
				...
				<th class="listdata">Contract Name</th>
				<th class="listdata">Route Name</th>
				<th class="listdata">Route Run</th>
				<th class="listdata">Route Time</th>
				<th class="listdata">Driver First Name</th>
				<th class="listdata">Driver Last Name</th>
				...
				<c:choose>
	      		<c:when test="${fn:length(entries) > 0}">
		      		<c:forEach items="${entries}" var="rule">
	 					...
	 					<tr>
							...
							<td class="field_content">${rule.companyName}</td>
	      					...
	      					<td class="field_content">${rule.driverFirstName}</td>
	      					<td class="field_content">${rule.driverLastName}</td>
	      					<td class="field_content">${rule.plateNumber}</td>
	      					<td class="field_content">${rule.serialNumber}</td>
	      				</tr>
	        		</c:forEach>
	        	</c:when>
	        	<c:otherwise>
