...
$(document).ready(function() {
	...
	<div id="printJob">	
	<div class="BCSHeaderText">View Archive Records For ${conname}</div>	
		 ...
		 <br />
    		...
    		<div class="alert alert-success" id="details_success_message" style="display:none;margin-top:10px;margin-bottom:10px;padding:5px;"></div>
			 <div id="BCS-Search">
			 Below is a table of the archive records for <b>${conname}</b> as requested. The first record is the current information for that employee.
			 ...
			 To export this data to Excel, CSV or to print/copy, you can select either option below to download a copy to open or print.
				...
				<tbody>
					...
					<c:set var="countCont" value="${countCont + 1}" />
							<tr>
							<td class="field_content">${countCont}</td>
							...
							<td class="field_content">${rule.value.workPhone}</td>
							<td class="field_content">${rule.value.company}</td>
							<td class="field_content">${rule.value.statusText}</td>
							<td class="field_content">${rule.value.businessNumber}</td>
							<td class="field_content">${rule.value.hstNumber}</td>
							<td class="field_content">${rule.value.comBean.tRegular}</td>
							<td class="field_content">${rule.value.comBean.tAlternate}</td>
							<td class="field_content">${rule.value.comBean.tParent}</td>
							<td class="field_content">${rule.value.comBean.crLastName}</td>
							<td class="field_content">${rule.value.comBean.crFirstName}</td>
							<td class="field_content">${rule.value.comBean.crPhoneNumber}</td>
							<td class="field_content">${rule.value.comBean.crEmail}</td>
							<td class="field_content">${rule.value.comBean.toLastName}</td>
							<td class="field_content">${rule.value.comBean.toFirstName}</td>
							<td class="field_content">${rule.value.comBean.toPhoneNumber}</td>
							<td class="field_content">${rule.value.comBean.toEmail}</td>
							</tr>
					</c:forEach>
				</tbody>
				</table>
