	...
	public String handleRequest(HttpServletRequest request, HttpServletResponse response)
		...
		if (supervisor == null) {
			...
			if ((claim_type != null) && (claim_type.equals("o"))) { //this is a regular monthly claim.
				int claim_id = TravelClaimDB.addClaim(usr.getPersonnel(), supervisor, form.get("year"),
						Integer.parseInt(form.get("month")));
				String xml = null;
				StringBuffer sb = new StringBuffer("<?xml version='1.0' encoding='ISO-8859-1'?>");
				sb.append("<TRAVELCLAIMS>");
				sb.append("<TRAVELCLAIM>");
				...
				int month = -1;
				try {
					pd = new PersonnelPD(form.get("title"), form.get("desc"), sdf.parse(form.get("sdate")), sdf.parse(
							form.get("fdate")));
