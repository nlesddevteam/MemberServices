...
public class GetTravelClaimsRejectedByLetterAjaxRequestHandler extends RequestHandlerImpl{
		...
		LinkedHashMap<Integer, Vector<TravelClaim>> travelclaims = null;
			try {
				if(searchletter.equals("All")){
					travelclaims = TravelClaimDB.getClaimsRejectedTreeMap();
				}else{
					travelclaims = TravelClaimDB.getClaimsRejectedLetterTreeMap(searchletter);
				}
				        	...
				        	Iterator p_iter = ((Vector)item.getValue()).iterator();
				           	while(p_iter.hasNext()){
				           		...
				           		if(!(claim.getClass().getName().toString().equals("com.awsd.travel.PDTravelClaim"))){
									...
									sb.append("<CLAIMDATE>" + Utils.getMonthString(claim.getFiscalMonth()) + "," + Utils.getYear(claim.getFiscalMonth(), claim.getFiscalYear()) +  "</CLAIMDATE>");
