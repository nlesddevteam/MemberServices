...
public class EditTravelClaimItemRequestHandler implements RequestHandler
{
          ...
          if(item_old != null)
          {
            ...
            if(op != null)
            {
              if(op.equalsIgnoreCase("CONFIRM"))
              {
                  ...
                  try
                  {
                    item = new TravelClaimItem(sdf.parse(request.getParameter("item_date")),
                      request.getParameter("item_desc"), Integer.parseInt(request.getParameter("item_kms")),
                      Double.parseDouble(request.getParameter("item_meals")), Double.parseDouble(request.getParameter("item_lodging")),
                      Double.parseDouble(request.getParameter("item_other")), request.getParameter("item_departure_time"),
                      request.getParameter("item_return_time"));
                  }
                  ...
                  if(item != null)
                  {
                      ...
                      || (claim.getFiscalMonth() != date_chk.get(Calendar.MONTH)))
                    {
                      ...
                      request.setAttribute("msg", "Claim item date is invalid.<br>Only items for " 
                        + Utils.getMonthString(claim.getFiscalMonth()) + " " + Utils.getYear(claim.getFiscalMonth(), claim.getFiscalYear()) + " are valid.");
                    ...
                    {
                      request.setAttribute("RESULT", "FAILED");
                      request.setAttribute("msg", "Claim item date is in the future.<br>Only items on or before " + (new SimpleDateFormat("MMMM dd, yyyy").format((Calendar.getInstance()).getTime())) + " are valid.");

                      item.setItemID(item_old.getItemID());
                      request.setAttribute("FAILED_ITEM", item);
                      request.setAttribute("EDIT", new Boolean(true));
                    }
                    else if((item.getItemKMS() == 0) && (item.getItemMeals() == 0)
                      && (item.getItemLodging() == 0) && (item.getItemOther() == 0))
                    {
                        request.setAttribute("RESULT", "FAILED");
                        request.setAttribute("msg", "All claim item values are zero (0).<br> Please enter one or more values greater then zero (0).");
              ...
              else if(op.equalsIgnoreCase("CANCEL")); // do nothing
            ...
            request.setAttribute("TOTAL_CLAIMED", new Double(TravelClaimDB.getYearToDateTotalClaimed(claim.getPersonnel(), claim.getFiscalYear())));
