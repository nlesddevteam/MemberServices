...
public class MSDeleteSchoolRequesthandler extends RequestHandlerImpl {

	public MSDeleteSchoolRequesthandler() {
		...
		if (validate_form()) {
			...
			if (teachers.size() <= 0) {
				if (!SchoolDB.deleteSchool(s)) {
					request.setAttribute("msgERR", "Could not delete " + s.getSchoolName() + " from database!");
				}
				else {
					request.setAttribute("msgOK", s.getSchoolName() + " had " + cnt
							+ " teacher(s) reassigned. Deletion successful.");
				}
			}
			else {
				request.setAttribute("msgRR", s.getSchoolName() + " has " + teachers.size()
						+ " teacher(s) associated with it. Delete could not be performed!");
			}
		}
		else {
			request.setAttribute("msgERR", this.validator.getErrorString());
		}
