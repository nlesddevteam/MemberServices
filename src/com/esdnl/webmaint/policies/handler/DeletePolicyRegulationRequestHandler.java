...
public class DeletePolicyRegulationRequestHandler implements RequestHandler {
	...
	public String handleRequest(HttpServletRequest request, HttpServletResponse response)
			...
			try {
				if (PolicyRegulationDB.deletePolicyRegulation(request.getParameter("cat").toUpperCase(),
					...
					File f = new File(session.getServletContext().getRealPath("/")
							+ "/../../nlesdweb/WebContent/about/policies/esd/regulations/" + request.getParameter("cat").toUpperCase()
							+ "_" + request.getParameter("code").toUpperCase() + "_" + request.getParameter("r_code").toUpperCase()
							+ ".pdf");
		...
		try {
			request.setAttribute("POLICY_REGULATIONS", PolicyRegulationDB.getPolicyRegulations(
					request.getParameter("cat").toUpperCase(), request.getParameter("code").toUpperCase()));
		}
		catch (SQLException e) {
			request.setAttribute("edit_msg", "Regulations for policy " + request.getParameter("cat") + "-"
					+ request.getParameter("code") + " could not be retrieved.");
		}
