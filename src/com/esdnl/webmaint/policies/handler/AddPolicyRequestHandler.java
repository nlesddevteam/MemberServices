            ...
            else
            {
              ...
              else
              {
                ...
                else
                {
                  try
                  {
                    PolicyDB.addPolicy(new Policy(mrequest.getParameter("cat_code").toUpperCase(), 
                      mrequest.getParameter("pol_code").toUpperCase(), mrequest.getParameter("pol_title").toUpperCase()));
                    
                    bean = new UploadBean();
                    bean.setFolderstore(session.getServletContext().getRealPath("/") + "../ROOT/about/policies/esd/");
                    bean.store(mrequest, "filedata");
                    f = new File(session.getServletContext().getRealPath("/") + "../ROOT/about/policies/esd/" + file.getFileName());
                    f.renameTo(new File(session.getServletContext().getRealPath("/") + "../ROOT/about/policies/esd/"
                      + mrequest.getParameter("cat_code").toUpperCase() + "_" + mrequest.getParameter("pol_code").toUpperCase() 
                      + ".pdf"));
