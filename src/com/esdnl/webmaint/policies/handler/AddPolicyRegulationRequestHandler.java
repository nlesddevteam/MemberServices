							...
							if ((files == null) || (files.size() < 1)) {
								...
								if ((file.getFileName().indexOf(".pdf") < 0) && (file.getFileName().indexOf(".PDF") < 0)) {
									...
									try {
										...
										PolicyRegulationDB.addPolicyRegulation(
												new PolicyRegulation(pol_code[0], pol_code[1], mrequest.getParameter(
														"reg_code").toUpperCase(), mrequest.getParameter("reg_title").toUpperCase()));

										bean = new UploadBean();
										bean.setFolderstore(session.getServletContext().getRealPath("/")
												+ "/../../nlesdweb/WebContent/about/policies/esd/regulations/");
										bean.store(mrequest, "filedata");
										f = new File(session.getServletContext().getRealPath("/")
												+ "/../../nlesdweb/WebContent/about/policies/esd/regulations/" + file.getFileName());

										f.renameTo(new File(session.getServletContext().getRealPath("/")
												+ "/../../nlesdweb/WebContent/about/policies/esd/regulations/" + pol_code[0] + "_" + pol_code[1]
												+ "_" + mrequest.getParameter("reg_code").toUpperCase() + ".pdf"));
										...
										case 1:
											request.setAttribute("msg",
													"POLICY REGULATION" + mrequest.getParameter("reg_code") + " ALREADY EXISTS");
											break;
										default:
