...
public class IncidentSchoolSafetyIssueService {

	public static ArrayList<SchoolSafetyIssueType> getIncidentSchoolSafetyIssues(IncidentBean abean)
		...
		try {
			...
			stat = con.prepareCall("begin ? := awsd_user.bullying_pkg.get_incident_safetyissues(?); end;");
			...
			System.err.println("ArrayList<SchoolSafetyIssueType> getIncidentSchoolSafetyIssues(IncidentBean abean): " + e);
			throw new BullyingException("Can not fetch Incident SchoolSafetyIssueType to DB.", e);
		}
		finally {
			try {
				stat.close();
	...
	public static void addIncidentSchoolSafetyIssues(IncidentBean abean) throws BullyingException {
		...
		try {
			...
			for (SchoolSafetyIssueType type : abean.getSchoolSafetyIssueTypes()) {
			...
			stat = con.prepareCall("begin  awsd_user.bullying_pkg.add_incident_safetyissues(?,?,?); end;");
			...
			abean.setSchoolSafetyIssueTypes(getIncidentSchoolSafetyIssues(abean));
		}
		catch (SQLException e) {
			e.printStackTrace();
			try {
			...
			System.err.println("void addIncidentSchoolSafetyIssues(IncidentBean abean): " + e);
