...
public class ThreateningBehaviorTypeCheckboxListTagHandler extends TagSupport {

	private static final long serialVersionUID = -5901352348568950336L;
	private String id;
	private String cls;
	private String style;
	private ArrayList<ThreateningBehaviorType> value;
	...
	public void setValue(ArrayList<ThreateningBehaviorType> value) {
	...
	public int doStartTag() throws JspException {
		...
		JspWriter out = null;
		HashMap<Integer, ThreateningBehaviorType> valueMap = new HashMap<Integer, ThreateningBehaviorType>();

		try {
			out = pageContext.getOut();

			if (this.value != null && this.value.size() > 0) {
				for (ThreateningBehaviorType type : this.value) {
			...
			for (ThreateningBehaviorType type : ThreateningBehaviorTypeService.getThreateningBehaviorTypes()) {
				...
				if (type.isIsSpecified()) {
					...
					out.print("\" "
							+ (valueMap.containsKey(type.getTypeId()) ? "value=\""
									+ ((ThreateningBehaviorType) valueMap.get(type.getTypeId())).getSpecified() + "\"" : "") + "/>");
				}
				out.println("<br/>");
			}
		}
