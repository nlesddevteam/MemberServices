...
public class SchoolSafetyIssueTypeCheckboxListTagHandler extends TagSupport {

	private static final long serialVersionUID = -5901352348568950336L;
	private String id;
	private String cls;
	private String style;
	private ArrayList<SchoolSafetyIssueType> value;
	...
	public void setValue(ArrayList<SchoolSafetyIssueType> value) {
	...
	public int doStartTag() throws JspException {
		...
		JspWriter out = null;
		HashMap<Integer, SchoolSafetyIssueType> valueMap = new HashMap<Integer, SchoolSafetyIssueType>();

		try {
			out = pageContext.getOut();

			if (this.value != null && this.value.size() > 0) {
				for (SchoolSafetyIssueType type : this.value) {
			...
			for (SchoolSafetyIssueType type : SchoolSafetyIssueTypeService.getSchoolSafetyIssueTypes()) {
				...
				if (type.isIsSpecified()) {
					...
					out.print("\" "
							+ (valueMap.containsKey(type.getTypeId()) ? "value=\""
									+ ((SchoolSafetyIssueType) valueMap.get(type.getTypeId())).getSpecified() + "\"" : "") + "/>");
				}
				out.println("<br/>");
			}
		}
