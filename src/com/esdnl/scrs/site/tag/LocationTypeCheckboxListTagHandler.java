...
public class LocationTypeCheckboxListTagHandler extends TagSupport {
	...
	private ArrayList<LocationType> value;
	...
	public void setValue(ArrayList<LocationType> value) {
	...
	public int doStartTag() throws JspException {
		...
		JspWriter out = null;
		HashMap<Integer, LocationType> valueMap = new HashMap<Integer, LocationType>();

		try {
			out = pageContext.getOut();

			if (this.value != null && this.value.size() > 0) {
				for (LocationType type : this.value) {
			...
			for (LocationType type : LocationTypeService.getLocationTypes()) {
				...
				if (type.isIsSpecified()) {
					...
					out.print("\" "
							+ (valueMap.containsKey(type.getTypeId()) ? "value=\""
									+ ((LocationType) valueMap.get(type.getTypeId())).getSpecified() + "\"" : "") + "/>");
				}
				out.println("<br/>");
			}
		}
