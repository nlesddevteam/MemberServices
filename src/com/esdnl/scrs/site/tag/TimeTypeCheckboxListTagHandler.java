...
public class TimeTypeCheckboxListTagHandler extends TagSupport {
	...
	private ArrayList<TimeType> value;
	...
	public void setValue(ArrayList<TimeType> value) {
	...
	public int doStartTag() throws JspException {
		...
		JspWriter out = null;
		HashMap<Integer, TimeType> valueMap = new HashMap<Integer, TimeType>();

		try {
			out = pageContext.getOut();

			if (this.value != null && this.value.size() > 0) {
				for (TimeType type : this.value) {
			...
			for (TimeType type : TimeTypeService.getTimeTypes()) {
				...
				if (type.isIsSpecified()) {
					...
					out.print("\" "
							+ (valueMap.containsKey(type.getTypeId()) ? "value=\""
									+ ((TimeType) valueMap.get(type.getTypeId())).getSpecified() + "\"" : "") + "/>");
				}
				out.println("<br/>");
			}
		}
