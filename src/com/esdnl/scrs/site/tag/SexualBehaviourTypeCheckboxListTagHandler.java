...
public class SexualBehaviourTypeCheckboxListTagHandler extends TagSupport {

	private static final long serialVersionUID = -5901352348568950336L;
	private String id;
	private String cls;
	private String style;
	private ArrayList<SexualBehaviourType> value;
	...
	public void setValue(ArrayList<SexualBehaviourType> value) {
	...
	public int doStartTag() throws JspException {
		...
		JspWriter out = null;
		HashMap<Integer, SexualBehaviourType> valueMap = new HashMap<Integer, SexualBehaviourType>();

		try {
			out = pageContext.getOut();

			if (this.value != null && this.value.size() > 0) {
				for (SexualBehaviourType type : this.value) {
			...
			for (SexualBehaviourType type : SexualBehaviourTypeService.getSexualBehaviourTypes()) {
				...
				if (type.isIsSpecified()) {
					...
					out.print("\" "
							+ (valueMap.containsKey(type.getTypeId()) ? "value=\""
									+ ((SexualBehaviourType) valueMap.get(type.getTypeId())).getSpecified() + "\"" : "") + "/>");
				}
				out.println("<br/>");
			}
		}
