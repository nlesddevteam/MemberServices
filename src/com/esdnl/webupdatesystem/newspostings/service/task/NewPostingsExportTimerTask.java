							...
							for (NewsPostingsBean np : postings) {
								...
								if(counter == 1) {
									...
									//Write format for Important Notice	(Type 5)
										...
										writer.println("<div class='importantNoticeDate siteFootNotes'>" + sdf.format(np.getNewsDate()) + "</div>");
												if(np.getNewsLocation() != null) {
													if(!(StringUtils.isEmpty(np.getNewsLocation().getLocationDescription()))){
														writer.println("<div class='importantNoticeLocation siteFootNotes'>" + np.getNewsLocation().getLocationDescription() + "</div>");
													}
												}
										...
										if(!(StringUtils.isEmpty(np.getNewsDocumentation()))){
											writer.println("<a href='/includes/files/news/doc/" + np.getNewsDocumentation() +"' title='View Attachment'>");
											writer.println("<img src='/includes/img/mini-icons/attachment-off.png' alt='View Attachment' border=0>");
											writer.println("</a>");
										}
										if(!(StringUtils.isEmpty(np.getNewsExternalLinkTitle()))){
											writer.println("<a href='" + np.getNewsExternalLink() + "' title='View External Link" + np.getNewsExternalLinkTitle() + "'>");
											writer.println("<img src='/includes/img/mini-icons/weblink-off.png' border=0 alt='" + np.getNewsExternalLinkTitle() + "'>");
											writer.println("</a>");
										}
										for(NewsPostingFileBean  npfb : np.getOtherNewsFiles()){
											...
											if(npfb.getNfTitle().toLowerCase().contains("form")) {
												writer.println("<img src='/includes/img/mini-icons/forms-off.png' alt='" + npfb.getNfTitle() + "' border=0>");
											}else if(npfb.getNfTitle().toLowerCase().contains("presentation")) {
												writer.println("<img src='/includes/img/mini-icons/presentation-off.png' alt='" + npfb.getNfTitle() + "' border=0>");
											}else {
												writer.println("<img src='/includes/img/mini-icons/doc-off.png' alt='" + npfb.getNfTitle() + "' border=0>");
											}
											writer.println("</a>");
										}
									...
									writer.println("<div class='newsFPTitle siteSubHeaders' style='font-weight: bold;'>" + np.getNewsTitle() +"</div>");
									if(np.getNewsLocation() != null) {
										if(!(StringUtils.isEmpty(np.getNewsLocation().getLocationDescription()))){
											writer.println("<div class='newsFPLocation siteFootNotes'>" + np.getNewsLocation().getLocationDescription() + "</div>");
										}
									}
									writer.println("<div class='newsFPBody siteBodyText'>");
									if(!(StringUtils.isEmpty(np.getNewsPhoto()))){
										writer.println("<img src='/includes/files/news/img/" + np.getNewsPhoto()+ "' class='newsFPIcon' border='0' title='" + np.getNewsPhotoCaption() + "alt=' "
									...
									writer.println("</div>");
									writer.println("<a href='/about/news/index.jsp?story=" + np.getNewsTitle() + "&NewsCategory=" + type.getValue() + "' title='Read Full Story'>");
									writer.println("<img src='/includes/img/mini-icons/fullstory-off.png' border=0 alt='Read Full Story'></a>");
									if(!(StringUtils.isEmpty(np.getNewsDocumentation()))){
										writer.println("<a href='/includes/files/news/doc/" + np.getNewsDocumentation() +"' title='View Attachment'>");
										writer.println("<img src='/includes/img/mini-icons/attachment-off.png' alt='View Attachment' border=0>");
										writer.println("</a>");
