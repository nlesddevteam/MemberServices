...
public class NLESDReferenceSSManageManager {
	public static NLESDReferenceSSManageBean addNLESDReferenceSSManageBean(NLESDReferenceSSManageBean abean) throws NLESDReferenceException {
		Connection con = null;
		CallableStatement stat = null;
		try {
			con = DAOUtils.getConnection();
			con.setAutoCommit(true);
			stat = con.prepareCall("begin ? := awsd_user.personnel_jobs_pkg.add_ref_chk_ss_man(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?); end;");
			...
			stat.setString(33, abean.getScale15());
			stat.setString(34,abean.getProviderEmail());
			stat.execute();
			...
			System.err.println("NLESDReferenceSSManageBean addNLESDReferenceSSSupportBean(NLESDReferenceSSManageBean abean): " + e);
			throw new NLESDReferenceException("Can not add NLESDReferenceSSManageBean to DB.", e);
		}
		finally {
			try {
				stat.close();
	...
	public static NLESDReferenceSSManageBean getNLESDReferenceSSManageBean(int id) throws NLESDReferenceException {
		NLESDReferenceSSManageBean eBean = null;
		Connection con = null;
		CallableStatement stat = null;
		ResultSet rs = null;
		try {
			con = DAOUtils.getConnection();
			stat = con.prepareCall("begin ? := awsd_user.personnel_jobs_pkg.get_nlesd_ref_chk_req_man(?); end;");
			stat.registerOutParameter(1, OracleTypes.CURSOR);
			stat.setInt(2, id);
			stat.execute();
			rs = ((OracleCallableStatement) stat).getCursor(1);
			if (rs.next())
				eBean = createNLESDReferenceSSManageBean(rs);
		}
		catch (SQLException e) {
			System.err.println("NLESDReferenceSSManageBean getNLESDReferenceSSManageBean(int id): " + e);
			throw new NLESDReferenceException("Can not extract NLESDReferenceSSManageBean from DB.", e);
		}
		finally {
			try {
				rs.close();
	...
	public static NLESDReferenceSSManageBean createNLESDReferenceSSManageBean(ResultSet rs) {
		NLESDReferenceSSManageBean abean = null;
		try {
			abean = new NLESDReferenceSSManageBean();
	...
	public static NLESDReferenceSSManageBean getLatestNLESDReferenceSSManageBean(String sin ) throws NLESDReferenceException {
		NLESDReferenceSSManageBean eBean = null;
		Connection con = null;
		CallableStatement stat = null;
		ResultSet rs = null;
		try {
			con = DAOUtils.getConnection();
			stat = con.prepareCall("begin ? := awsd_user.personnel_jobs_pkg.get_nlesd_ref_chk_req_man_l(?); end;");
			stat.registerOutParameter(1, OracleTypes.CURSOR);
			...
			System.err.println("NLESDReferenceSSManageBean getLatestNLESDReferenceSSManageBean(String sin ): " + e);
			throw new NLESDReferenceException("Can not extract NLESDReferenceSSManageBean from DB.", e);
		}
		finally {
			try {
