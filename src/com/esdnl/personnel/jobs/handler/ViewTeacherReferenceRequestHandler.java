...
public class ViewTeacherReferenceRequestHandler extends RequestHandlerImpl {

	public ViewTeacherReferenceRequestHandler() {
	...
	public String handleRequest(HttpServletRequest request, HttpServletResponse response)
		...
		if (validate_form()) {
			try {
				ReferenceBean ref = ReferenceManager.getReferenceBean(form.getInt("id"));
				...
				if (usr.checkRole("PRINCIPAL") || usr.checkRole("VICE PRINCIPAL")) {
					...
					if (ref.getReferenceProviderName().equalsIgnoreCase(p.getFullName())
							|| ref.getReferenceProviderName().equalsIgnoreCase(usr.getPersonnel().getFullName()))
						authorized = true;
					else {
						for (JobOpportunityBean opp : JobOpportunityManager.getJobOpportunityBeans(p.getSchool().getSchoolID())) {
							if (ApplicantProfileManager.getApplicantShortlistMap(opp).containsKey(ref.getApplicant().getUID())) {
								authorized = true;
								break;
							}
						}
