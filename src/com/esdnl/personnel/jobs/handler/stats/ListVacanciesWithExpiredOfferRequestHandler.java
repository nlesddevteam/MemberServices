...
public class ListVacanciesWithExpiredOfferRequestHandler extends RequestHandlerImpl {

	public ListVacanciesWithExpiredOfferRequestHandler() {
			...
			if (validate_form()) {
				...
				if (zone != null) {
					request.setAttribute("VACANCIES",
							TeacherAllocationVacancyStatisticsManager.getVacanciesWithExpiredOffer(form.get("sy"), zone));
					request.setAttribute("VacancyListTitle",
							"Vacancies with Expired Offer - " + StringUtils.capitaliseAllWords(zone.getZoneName()) + " Region");
