...
public class ListVacanciesWithRejectedOfferRequestHandler extends RequestHandlerImpl {

	public ListVacanciesWithRejectedOfferRequestHandler() {
			...
			if (validate_form()) {
				...
				if (zone != null) {
					request.setAttribute("VACANCIES",
							TeacherAllocationVacancyStatisticsManager.getVacanciesWithRejectedOffer(form.get("sy"), zone));
					request.setAttribute("VacancyListTitle",
							"Vacancies with Rejected Offer - " + StringUtils.capitaliseAllWords(zone.getZoneName()) + " Region");
