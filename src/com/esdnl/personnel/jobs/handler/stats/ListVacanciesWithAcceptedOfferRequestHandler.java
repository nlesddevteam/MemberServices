...
public class ListVacanciesWithAcceptedOfferRequestHandler extends RequestHandlerImpl {

	public ListVacanciesWithAcceptedOfferRequestHandler() {
			...
			if (validate_form()) {
				...
				if (zone != null) {
					request.setAttribute("VACANCIES",
							TeacherAllocationVacancyStatisticsManager.getVacanciesWithAcceptedOffer(form.get("sy"), zone));
					request.setAttribute("VacancyListTitle",
							"Vacancies with Accepted Offer - " + StringUtils.capitaliseAllWords(zone.getZoneName()) + " Region");
