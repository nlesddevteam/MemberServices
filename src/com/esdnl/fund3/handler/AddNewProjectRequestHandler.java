...
public class AddNewProjectRequestHandler extends RequestHandlerImpl {
	public AddNewProjectRequestHandler() {
	...
	public String handleRequest(HttpServletRequest request, HttpServletResponse reponse)
		...
		try {
			...
			if(form.get("op") == null)
			{
				...
				pb.setStatusBy(usr.getPersonnel().getFullNameReverse());
				...
				alb.setLogEntry("New Project Created : [" + pb.getProjectName() + "]");
				alb.setProjectId(id);
				AuditLogManager.addNewAuditLog(alb);
				//add audit log for position responsibile
				//now we update the audit log
				alb.setUserName(usr.getPersonnel().getFullNameReverse());
				alb.setLogEntry("Added Position Responsible For Project: [" + pb.getEmployeeName()  + "]");
				alb.setProjectId(id);
				AuditLogManager.addNewAuditLog(alb);
				...
				//save an arraylist of the regions to use with EmailManager
				ArrayList<String> emailregions= new ArrayList<String>();
				...
				//now we email the all the users who should be notified
				//first the regional managers
				Fund3EmailManager.sendAddProjectEmails(emailregions, pb);
				//next we send the approval email to the AD for each region or provincal ad
				Fund3EmailManager.sendAddProjectEmailsApprovals(emailregions, pb);
				//next we send new project message to senior accountant
				Fund3EmailManager.sendAddProjectEmailSeniorAcct(pb);
				...
				TreeMap<String,Double> tm = ProjectRegionManager.getProjectRegionByIdEdit(id);
				request.setAttribute("regionbud", tm);
				//add the regions that were used
				TreeMap<String,Double> tmm = ProjectRegionManager.getProjectRegionsByProject(id);
				request.setAttribute("regionbudused", tmm);
				//regions=3
				ArrayList<DropdownItemBean> regitems = DropdownManager.getDropdownItems(3);
				request.setAttribute("regions", regitems);
				request.setAttribute("msg", "Project has been added!");
